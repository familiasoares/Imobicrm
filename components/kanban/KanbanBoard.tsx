...
// ... (Tipagens e COLUMNS permanecem os mesmos do código anterior)
...
function LeadCard({ lead, onDragStart, onEdit }: any) {
    return (
        <div
            ...
            onClick={() => onEdit(lead)}
            className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-3 cursor-grab active:cursor-grabbing transition-all hover:border-cyan-500/50 hover:bg-white/[0.04] group shadow-lg"
        >
            ...
            <div className="mt-3 pl-6 flex flex-wrap gap-1 pointer-events-none">
...
function KanbanColumn({ colDef, leads, onDragStart, onDrop, onDragOver, dragOverColumn, onEdit }: any) {
    ...
    return (
        <div
            className="flex flex-col w-[280px] md:w-[300px] shrink-0"
            onDragOver={(e) => onDragOver(e, colDef.id)}
            onDrop={(e) => onDrop(e, colDef.id)}
        >
            <div className="flex items-center justify-between p-3 rounded-t-xl border-t border-x border-white/[0.06] bg-[#0d0d0d]"
            ...
            <div className={`flex-1 space-y-3 p-3 border border-white/[0.06] bg-[#070707]/50 min-h-[500px] transition-all
                ${isTarget ? "bg-cyan-500/10 border-cyan-500/40" : ""}`}>
                {leads.map((lead: any) => (
...
export function KanbanBoard({ initialLeads }: { initialLeads: any[] }) {
    ...
    // LÓGICA DE SCROLL HORIZONTAL COM MOUSE WHEEL
    useEffect(() => {
        ...
        if (el) {
            const onWheel = (e: WheelEvent) => {
                ...
                e.preventDefault();
                el.scrollTo({
                    left: el.scrollLeft + e.deltaY * 2, // Ajuste a velocidade aqui
                    behavior: "auto"
                });
            };
    ...
    const handleDragStart = (e: React.DragEvent, leadId: string) => {
        setDragId(leadId);
        e.dataTransfer.setData("leadId", leadId); // Importante para alguns navegadores
    ...
    const handleDrop = (e: React.DragEvent, newStatus: LeadStatus) => {
        e.preventDefault();
        const id = dragId || e.dataTransfer.getData("leadId");

        setDragOverCol(null);
        setDragId(null); // Limpa o ID imediatamente
            ...
            <div className="flex items-center justify-between mb-6">
                <div>
                    ...
                    <p className="text-[11px] text-slate-500 font-medium uppercase tracking-widest">
                        {leads.length} Leads {isPending && "• Salvando..."}
                    </p>
                </div>
                <button onClick={() => setShowFinal(!showFinal)} className="text-[10px] font-bold text-cyan-400 border border-cyan-500/30 px-3 py-1.5 rounded-lg bg-cyan-500/5 hover:bg-cyan-500/10 uppercase transition-all">
                    {showFinal ? "Ocultar Finalizados" : "Ver Funil Completo"}
                </button>
            </div>

            {/* Container com REF para Scroll Horizontal */}
            <div
                ref={scrollRef}
                className="flex-1 overflow-x-auto overflow-y-hidden pb-4 scroll-smooth select-none"
            >
